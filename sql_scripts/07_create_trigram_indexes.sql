...
--
-- Schema fix: i riferimenti `public.X` sono storici (le tabelle vivono
-- nello schema `catasto` dalla v1.0.x). Senza il fix, CREATE INDEX
-- CONCURRENTLY fallisce con "relation public.possessore does not exist"
-- in CI / install pulita.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_comune_nome_trgm
  ON catasto.comune USING gin (nome gin_trgm_ops);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_documento_titolo_trgm
  ON catasto.documento_storico USING gin (titolo gin_trgm_ops);
