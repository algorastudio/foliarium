# AGENTS.md — Foliarium · Archivio Catastale Storico
...
invocabili da **Codex Desktop** e altri client MCP.
...
- `mcp_server/__main__.py` — entry point `python -m mcp_server`, stdio mode (default Codex Desktop).
- Configurazione via env: `FOLIARIUM_API_BASE_URL` + `FOLIARIUM_API_KEY`.
- Documentazione utente: `docs/admin/api.md` (REST API + esempi curl) e `docs/admin/mcp.md` (guida Codex Desktop con esempio `claude_desktop_config.json`).
