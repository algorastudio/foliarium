...
class DBBackupMixin:
    ...
    def register_backup_log(
        ...
        esito: bool,
        percorso_file: str,
        dimensione_bytes: Optional[int] = None,
        ...
        try:
            ...
            if backup_id:
                self.logger.info(
                    f"Log backup registrato con ID {backup_id} per '{nome_file}'"
                )
            else:
                self.logger.error(
                    f"registra_backup non ha restituito un ID per '{nome_file}'."
                )
            return backup_id
        except psycopg2.Error as db_err:
            self.logger.error(f"Errore DB reg log backup '{nome_file}': {db_err}")
        except Exception as e:
            self.logger.error(f"Errore Python reg log backup '{nome_file}': {e}")
        return None
    ...
    def get_backup_command_parts(
        ...
        actual_pg_dump_path = self._resolve_executable_path(
            pg_dump_executable_path_ui, "pg_dump.exe"
        )
        ...
        if not all([db_user, db_host, db_port, db_name]):
            self.logger.error(
                "Parametri di connessione mancanti per il backup (da _main_db_conn_params)."
            )
            return None
    ...
    def get_restore_command_parts(
        self, backup_file_path: str, pg_tool_executable_path_ui: str
    ) -> Optional[List[str]]:
        # USA L'ATTRIBUTO CORRETTO: _main_db_conn_params
        db_user = self._main_db_conn_params.get("user")
        db_host = self._main_db_conn_params.get("host")
        ...
        if not all([db_user, db_host, db_port, db_name]):
            self.logger.error(
                "Parametri di connessione mancanti per il ripristino (da _main_db_conn_params)."
            )
            return None
        ...
        if file_extension in [".dump", ".backup", ".custom"]:
            actual_pg_tool_path = self._resolve_executable_path(
                pg_tool_executable_path_ui, "pg_restore.exe"
            )
            if not actual_pg_tool_path:
                return None
            command = [
                actual_pg_tool_path,
                "-U",
            ...
            )  # Opzioni comuni per pg_restore
            command.append(backup_file_path)
        elif file_extension == ".sql":
            actual_pg_tool_path = self._resolve_executable_path(
                pg_tool_executable_path_ui, "psql.exe"
            )
            if not actual_pg_tool_path:
                return None
            command = [
            ...
            command.extend(
                ["-f", backup_file_path, "-v", "ON_ERROR_STOP=1"]
            )  # Esegui script SQL con psql
        else:
            self.logger.error(
                f"Formato file di backup non riconosciuto o non supportato: '{file_extension}'"
            )
            return None
    ...
    def _resolve_executable_path(
        self, user_provided_path: str, default_name: str
    ) -> Optional[str]:
        if (
            user_provided_path
            and os.path.isabs(user_provided_path)
            and os.path.exists(user_provided_path)
            and os.path.isfile(user_provided_path)
        ):
            self.logger.info(
                f"Utilizzo del percorso eseguibile fornito: {user_provided_path}"
            )
            return user_provided_path
        elif user_provided_path:
            self.logger.warning(
                f"Percorso fornito '{user_provided_path}' per '{default_name}' non valido. Tento ricerca nel PATH."
            )

        found_path_in_system = shutil.which(default_name)
        if found_path_in_system:
            self.logger.info(
                f"Trovato eseguibile '{default_name}' nel PATH: {found_path_in_system}"
            )
            return found_path_in_system
        else:
            self.logger.error(
                f"Eseguibile '{default_name}' non trovato nel PATH e nessun percorso valido fornito."
            )
    ...
    def cleanup_old_backup_logs(self, giorni_conservazione: int = 30) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Pulizia log backup: eliminati record più vecchi di {giorni_conservazione} giorni."
            )
            return True
        except psycopg2.Error as db_err:
            self.logger.error(f"Errore DB pulizia log backup: {db_err}")
            return False
        except Exception as e:
            self.logger.error(f"Errore Python pulizia log backup: {e}")
            return False
    ...
    def generate_backup_script(self, backup_dir: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Errore DB generazione script backup: {db_err}")
            return None
        except Exception as e:
            self.logger.error(f"Errore Python generazione script backup: {e}")
            return None
    ...
    def execute_restore_from_file_emergency(
        ...
        db_user = self._main_db_conn_params.get("user")
        db_password = self._main_db_conn_params.get(
            "password"
        )  # Richiede la password per i tool
        db_host = self._main_db_conn_params.get("host")
        db_port = str(self._main_db_conn_params.get("port"))
        db_name = self._main_db_conn_params.get("dbname")
        ...
        for description, command in commands:
            ...
            process.setProcessEnvironment(
                self.create_clean_environment()
            )  # Usa un ambiente pulito
