...
class DBPartiteMixin:
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if result and result["partita_data"] is not None:
                    self.logger.info(
                        f"Dati esportazione recuperati per partita ID {partita_id}"
                    )
                    return result["partita_data"]
                else:
                    raise DBNotFoundError(
                        f"Partita ID {partita_id} non trovata o dato NULL"
                    )
    ...
    def _insert_partite_records(
        ...
        try:
                ...
                with conn.cursor() as cur:
                    for i, record in enumerate(records):
                        ...
                        try:
                            ...
                            if cur.fetchone():
                                suf = (
                                    f" con suffisso '{suffisso_partita}'"
                                    if suffisso_partita
                                    else ""
                                )
                                raise ValueError(
                                    f"La partita n.{numero_partita}{suf} esiste già."
                                )
                            cur.execute(
                                ...
                                (
                                    ...
                                    record.get("data_chiusura") or None,
            ...
            raise DBMError(
                f"Errore critico di sistema durante l'importazione: {e}"
            ) from e
        ...
        try:
            with open(file_path, mode="r", encoding="utf-8") as csvfile:
                ...
                if not required.issubset(reader.fieldnames or []):
                    raise ValueError(
                        f"Intestazioni mancanti. Richieste: {', '.join(required)}"
                    )
                for i, row in enumerate(reader):
                    if not all(row.get(k) for k in required):
                        raise ValueError(
                            f"Dati mancanti alla riga {i + 2}. Campi obbligatori: {', '.join(required)}."
                        )
    ...
    def import_partite_from_xlsx(
        ...
        try:
            ...
            if not required.issubset(set(headers)):
                raise ValueError(
                    f"Intestazioni mancanti nel foglio Excel. Richieste: {', '.join(required)}"
                )
            records: List[Dict] = []
            ...
            raise IOError(
                "La libreria 'openpyxl' non è installata. Esegui: pip install openpyxl"
            )
        except Exception as e:
    ...
    def create_partita(
        ...
        if not all([comune_id, numero_partita, tipo, stato, data_impianto]):
            raise DBDataError(
                "Comune, Numero Partita, Tipo, Stato e Data Impianto sono obbligatori."
            )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if result and result[0]:
                        self.logger.info(
                            f"Partita N.{numero_partita} creata con successo. ID: {result[0]}"
                        )
                        return result[0]
                    else:
                        raise DBMError(
                            "Creazione partita fallita, nessun ID restituito."
                        )
        except psycopg2.errors.UniqueViolation as e:
            # Rileva violazione del vincolo di unicità (comune_id, numero_partita, suffisso_partita)
            raise DBUniqueConstraintError(
                "Esiste già una partita con questo numero e suffisso nel comune selezionato."
            ) from e
        except Exception as e:
            self.logger.error(
                f"Errore DB durante la creazione della partita: {e}", exc_info=True
            )
            raise DBMError(
                f"Errore imprevisto durante la creazione della partita: {e}"
            ) from e

    @db_handle_errors
    def get_partite_by_comune(
        self, comune_id: int, filter_text: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def update_partita(self, partita_id: int, dati_modificati: Dict[str, Any]):
        ...
        set_clauses = [
            f"{field} = %s" for field in allowed_fields if field in dati_modificati
        ]
        params = [
            dati_modificati[field]
        ...
        if not set_clauses:
            self.logger.info(
                "Nessun campo valido fornito per l'aggiornamento della partita."
            )
            return
        ...
        query = (
            f"UPDATE {self.schema}.partita SET {', '.join(set_clauses)} WHERE id = %s;"
        )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        ...
                        raise DBNotFoundError(
                            f"Nessuna partita trovata con ID {partita_id} per l'aggiornamento."
                        )
            # Il commit è automatico qui
            self.logger.info(f"Partita ID {partita_id} aggiornata con successo.")
        except Exception as e:
            self.logger.error(
                f"Errore DB aggiornando partita ID {partita_id}: {e}", exc_info=True
            )
    ...
    def duplicate_partita(
        self,
        partita_id_originale: int,
        nuovo_numero_partita: int,
        mantenere_possessori: bool = True,
        mantenere_immobili: bool = False,
        nuovo_suffisso: Optional[str] = None,
    ) -> bool:
        ...
        params = (
            partita_id_originale,
            nuovo_numero_partita,
            mantenere_possessori,
            mantenere_immobili,
            nuovo_suffisso,
        )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    self.logger.info(
                        f"Tentativo di duplicare partita ID {partita_id_originale} in Nuovo N.{nuovo_numero_partita}"
                    )
                    cur.execute(call_proc_str, params)

            self.logger.info(
                f"Partita ID {partita_id_originale} duplicata con successo."
            )
            return True
        except psycopg2.Error as db_err:
            pgerror_msg = getattr(db_err, "pgerror", str(db_err))
            self.logger.error(
                f"Errore DB durante duplicazione partita ID {partita_id_originale}: {pgerror_msg}",
                exc_info=True,
            )
            raise DBMError(
                f"Errore database durante la duplicazione: {pgerror_msg}"
            ) from db_err
        except Exception as e:
            self.logger.error(
                f"Errore Python durante duplicazione partita ID {partita_id_originale}: {e}",
                exc_info=True,
            )
            raise DBMError(f"Errore di sistema durante la duplicazione: {e}") from e

    def registra_nuova_proprieta(
        self,
        comune_id: int,
        numero_partita: int,
        data_impianto: date,
        ...
        if not (isinstance(numero_partita, int) and numero_partita > 0):
            raise DBDataError("Numero partita non valido.")
        try:
        ...
        actual_suffisso_partita = (
            suffisso_partita.strip() if isinstance(suffisso_partita, str) else None
        )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    self.logger.debug(
                        f"Chiamata procedura registra_nuova_proprieta per C:{comune_id}, N:{numero_partita}"
                    )
                    ...
                    if result and result["id"]:
                        ...
                        self.logger.info(
                            f"Nuova proprietà registrata. Partita ID: {new_partita_id}."
                        )
                        return new_partita_id
                    else:
                        raise DBMError(
                            "Fallimento nel recuperare l'ID della nuova partita dopo la registrazione."
                        )
        ...
        # --- BLOCCO DI GESTIONE ECCEZIONI MIGLIORATO ---
            ...
            if constraint_name == "partita_unique_numero_suffisso_comune":
                ...
                raise DBUniqueConstraintError(
                    messaggio, constraint_name=constraint_name
                ) from uve
            else:
                ...
                raise DBUniqueConstraintError(
                    messaggio_generico, constraint_name=constraint_name
                ) from uve
    ...
    def registra_passaggio_proprieta(
        self,
        ...
        suffisso_nuova_partita: Optional[str] = None,
    ) -> bool:
        ...
        try:
            nuovi_possessori_jsonb = (
                json.dumps(nuovi_possessori_list) if nuovi_possessori_list else None
            )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    self.logger.info(
                        f"Tentativo di registrare passaggio proprietà da Partita ID {partita_origine_id}..."
                    )
                    cur.execute(call_proc_str, params)

            self.logger.info(
                "Passaggio di proprietà registrato con successo tramite procedura."
            )
            return True
        except psycopg2.Error as db_err:
            pgerror_msg = getattr(db_err, "pgerror", str(db_err))
            self.logger.error(
                f"Errore DB durante registrazione passaggio proprietà: {pgerror_msg}",
                exc_info=True,
            )
            raise DBMError(f"Errore database: {pgerror_msg}") from db_err
        except Exception as e:
            self.logger.error(
                f"Errore Python durante registrazione passaggio proprietà: {e}",
                exc_info=True,
            )
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if result and result[0] is not None:
                    ...
                    raise DBNotFoundError(
                        f"Nessun report generato per partita ID {partita_id}"
                    )
    ...
    def get_partite_complete_view(
        self,
        comune_id: Optional[int] = None,
        stato: Optional[str] = None,
        limit: int = 100,
    ) -> List[Dict]:  # Usa comune_id
        ...
        try:
            ...
            self.logger.error(f"Errore DB get_partite_complete_view: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_partite_complete_view: {e}")
            return []

    def aggiorna_legame_partita_possessore(
        self, partita_possessore_id: int, titolo: str, quota: Optional[str]
    ) -> bool:
        ...
        actual_quota = (
            quota.strip() if isinstance(quota, str) and quota.strip() else None
        )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        ...
                        raise DBNotFoundError(
                            f"Legame partita-possessore con ID {partita_possessore_id} non trovato."
                        )

            # Il commit è automatico se nessuna eccezione è stata sollevata
            self.logger.info(
                f"Legame partita-possessore ID {partita_possessore_id} aggiornato."
            )
            return True
            ...
            self.logger.error(
                f"Errore previsto aggiornando legame {partita_possessore_id}: {e}",
                exc_info=True,
            )
            raise e
        except Exception as e:
            ...
            self.logger.error(
                f"Errore imprevisto aggiornando legame {partita_possessore_id}: {e}",
                exc_info=True,
            )
            raise DBMError(f"Impossibile aggiornare il legame: {e}") from e

    def get_cronologia_variazioni(
        self,
        comune_origine_id: Optional[int] = None,
        tipo_variazione: Optional[str] = None,
        limit: int = 100,
    ) -> List[Dict]:  # Usa comune_id
        ...
        try:
            ...
            if comune_origine_id is not None:
                ...
                if tipo_variazione:
                    query += " AND m.tipo_variazione = %s"
                    params.append(tipo_variazione)
            elif tipo_variazione:
                query += " WHERE tipo_variazione = %s"
                params.append(tipo_variazione)
            ...
            self.logger.error(f"Errore DB get_cronologia_variazioni: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_cronologia_variazioni: {e}")
            return []
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if result and result["partita_json"]:
                    try:
                        return json.dumps(
                            result["partita_json"], indent=4, ensure_ascii=False
                        )
                    except (TypeError, ValueError) as json_err:
                        self.logger.error(
                            f"JSON serialization error for partita {partita_id}: {json_err}"
                        )
                        return str(result["partita_json"])
                else:
                    raise DBNotFoundError(
                        f"Nessun JSON restituito per partita ID {partita_id}"
                    )
    ...
    def get_report_annuale_partite(self, comune_id: int, anno: int) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Errore DB get_report_annuale_partite: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_report_annuale_partite: {e}")
            return []
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    conn.commit()
                    self.logger.info(
                        f"Tipo di possesso '{nome}' inserito (ID: {tipo_id})"
                    )
                    return tipo_id
                except psycopg2.IntegrityError as e:
                    conn.rollback()
                    raise DBUniqueConstraintError(
                        f"Tipo di possesso '{nome}' già esiste: {e}"
                    )

    @db_handle_errors
    def update_tipo_possesso(
        self, tipo_id: int, nome: str, descrizione: Optional[str] = None
    ) -> bool:
            ...
            with conn.cursor() as cur:
                try:
                    ...
                    if cur.fetchone() is None:
                        raise DBNotFoundError(
                            f"Tipo di possesso ID {tipo_id} non trovato."
                        )
                    conn.commit()
                    ...
                    conn.rollback()
                    raise DBUniqueConstraintError(
                        f"Nome '{nome}' già esiste per un altro tipo: {e}"
                    )
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    conn.rollback()
                    raise DBMError(
                        f"Impossibile eliminare il tipo di possesso (in uso): {e}"
                    )

    def get_report_proprieta_possessore(
        self, possessore_id: int, data_inizio: date, data_fine: date
    ) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Errore DB get_report_proprieta_possessore: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_report_proprieta_possessore: {e}")
            return []
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if row:
                    ...
                    raise DBNotFoundError(
                        f"Nessun report trovato per comune ID {comune_id}"
                    )
    ...
    def genera_report_proprieta(self, partita_id: int) -> Optional[str]:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if result and result[0] is not None:
                    self.logger.info(
                        f"Report di proprietà generato per partita ID {partita_id}"
                    )
                    return str(result[0])
                else:
                    raise DBNotFoundError(
