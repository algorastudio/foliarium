...
class DBDocumentiMixin:
    ...
    @db_handle_errors
    def aggiungi_documento_storico(
        self,
        titolo: str,
        tipo_documento: str,
        percorso_file: str,
        ...
        metadati_json: Optional[str] = None,
    ) -> int:
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if not result:
                    raise DBMError(
                        "Creazione del documento fallita, nessun ID restituito."
                    )
                return result["id"]

    def collega_documento_a_partita(
        self,
        documento_id: int,
        partita_id: int,
        rilevanza: str,
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    self.logger.info(
                        f"Collegamento doc ID {documento_id} a partita ID {partita_id}."
                    )
                    cur.execute(query, params)

            self.logger.info(
                "Documento collegato/aggiornato alla partita con successo."
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Errore DB collegando doc {documento_id} a partita {partita_id}: {e}",
                exc_info=True,
            )
    ...
    def scollega_documento_da_partita(self, documento_id: int, partita_id: int) -> bool:
        ...
        try:
                ...
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        raise DBNotFoundError(
                            f"Nessun legame trovato tra doc ID {documento_id} e partita ID {partita_id}."
                        )

            self.logger.info(
                f"Legame tra doc {documento_id} e partita {partita_id} rimosso."
            )
            return True
            ...
            self.logger.error(
                f"Errore DB scollegando doc {documento_id} da partita {partita_id}: {e}",
                exc_info=True,
            )
    ...
    def link_document_to_partita(
        self,
        document_id: int,
        partita_id: int,
        relevance: str = "correlata",
        ...
        try:
            ...
            self.logger.info(
                f"Link creato/aggiornato Doc {document_id} → Partita {partita_id}."
            )
            return True
        except psycopg2.Error as db_err:
            self.logger.error(f"Errore DB link doc-partita: {db_err}")
            return False
        except Exception as e:
            self.logger.error(f"Errore Python link doc-partita: {e}")
            return False
    ...
    def get_historical_name(
        self, entity_type: str, entity_id: int, year: Optional[int] = None
    ) -> Optional[Dict]:
        ...
        try:
            ...
            self.logger.error(
                f"Errore DB in get_historical_name ({entity_type} ID {entity_id}): {e}",
                exc_info=True,
            )
            return None

    def register_historical_name(
        self,
        entity_type: str,
        entity_id: int,
        name: str,
        ...
        call_proc = (
            f"CALL {self.schema}.registra_nome_storico(%s, %s, %s, %s, %s, %s, %s)"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Registrato nome storico '{name}' per {entity_type} ID {entity_id}."
            )
            return True
        except Exception as e:
            self.logger.error(
                f"Errore DB in register_historical_name: {e}", exc_info=True
            )
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                if result:
                    ...
                    raise DBNotFoundError(
                        f"Periodo storico con ID {periodo_id} non trovato."
                    )

    def update_periodo_storico(
        self, periodo_id: int, dati_modificati: Dict[str, Any]
    ) -> bool:
        ...
        for key, col in campi_permessi.items():
            if key in dati_modificati:
                ...
                if key == "nome" and not (valore and str(valore).strip()):
                    raise DBDataError(
                        "Il nome del periodo storico non può essere vuoto."
                    )
        ...
        if not set_clauses:
            self.logger.info(
                f"Nessun campo aggiornabile fornito per periodo ID {periodo_id}."
            )
            return True
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        raise DBNotFoundError(
                            f"Periodo storico con ID {periodo_id} non trovato o dati identici."
                        )

            self.logger.info(
                f"Periodo storico ID {periodo_id} aggiornato con successo."
            )
            return True
            ...
            self.logger.error(
                f"Errore previsto aggiornando periodo storico {periodo_id}: {e}",
                exc_info=True,
            )
            raise e  # Rilancia l'eccezione specifica
        except Exception as e:
            self.logger.error(
                f"Errore imprevisto DB aggiornando periodo storico {periodo_id}: {e}",
                exc_info=True,
            )
            raise DBMError(f"Impossibile aggiornare il periodo storico: {e}") from e

    def aggiungi_periodo_storico(
        self,
        nome: str,
        anno_inizio: int,
        ...
        if anno_fine is not None and anno_fine < anno_inizio:
            raise DBDataError(
                "L'anno di fine non può essere precedente a quello di inizio."
            )
        ...
        try:
            ...
            raise DBUniqueConstraintError(
                f"Un periodo storico con nome '{nome}' esiste già."
            ) from None
        except Exception as e:
            self.logger.error(
                f"Errore DB in aggiungi_periodo_storico: {e}", exc_info=True
            )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        raise DBNotFoundError(
                            f"Nessun periodo storico trovato con ID {periodo_id}."
                        )
                    return True
        except psycopg2.errors.ForeignKeyViolation:
            raise DBMError(
                "Impossibile eliminare: questo periodo è utilizzato da uno o più comuni."
            ) from None
        except Exception as e:
            self.logger.error(
                f"Errore DB in elimina_periodo_storico: {e}", exc_info=True
            )
            raise DBMError("Eliminazione del periodo storico fallita.") from e

    def get_cadastral_stats_by_period(
        self,
        comune_id: Optional[int] = None,
        year_start: int = 1900,  # Usa comune_id
        year_end: Optional[int] = None,
    ) -> List[Dict]:
        """Chiama la funzione SQL statistiche_catastali_periodo (MODIFICATA per comune_id)."""
        self.logger.warning(
            "La funzione SQL 'statistiche_catastali_periodo' potrebbe non essere aggiornata per comune_id."
        )
        try:
            if year_end is None:
            ...
            self.logger.warning(
                "Funzione 'statistiche_catastali_periodo' non trovata nel DB."
            )
            return []
        except psycopg2.Error as db_err:
            self.logger.error(f"Errore DB get_cadastral_stats_by_period: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_cadastral_stats_by_period: {e}")
            return []
        return []
