...
class DBPossessoriMixin:
    ...
    def import_possessori_from_csv(
        ...
        try:
            ...
            with open(file_path, mode="r", encoding="utf-8") as csvfile:
                ...
                if not required_headers.issubset(reader.fieldnames or []):
                    raise ValueError(
                        f"Intestazioni mancanti nel CSV. Richieste: {', '.join(required_headers)}"
                    )
                ...
                for i, row in enumerate(reader):
                    ...
                    if not row.get("cognome_nome") or not row.get("nome_completo"):
                        raise ValueError(
                            f"Dati mancanti alla riga {line_num}. 'cognome_nome' e 'nome_completo' sono obbligatori."
                        )
        ...
        try:
                    ...
                    for i, record in enumerate(records_to_import):
                        ...
                        try:
                            ...
                            if cur.fetchone():
                                ...
                                raise ValueError(
                                    f"Il possessore '{nome_completo}' esiste già in questo comune."
                                )
                            ...
                            if not new_id_result:
                                raise DBMError(
                                    "Inserimento fallito, nessun ID restituito dal database."
                                )
            ...
            self.logger.error(
                f"Errore critico durante l'importazione CSV dei possessori: {e}",
                exc_info=True,
            )
    ...
    @db_handle_errors
    def check_possessore_exists(
        self, nome_completo: str, comune_id: Optional[int] = None
    ) -> Optional[int]:
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not result:
                        raise DBMError(
                            "Creazione possessore fallita, nessun ID restituito."
                        )
                    return result[0]
        except psycopg2.errors.UniqueViolation as e:
            raise DBUniqueConstraintError(
                "Un possessore con questi dati esiste già.", details=str(e)
            ) from e
        except Exception as e:
    ...
    @db_handle_errors
    def search_possessori_by_term_globally(
        self, search_term: Optional[str], limit: int = 200, solo_attivi: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        if search_term and search_term.strip():
            ...
            where_clauses.append(
                "(p.nome_completo ILIKE %s OR p.cognome_nome ILIKE %s OR p.paternita ILIKE %s)"
            )
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                self.logger.info(
                    f"search_possessori_by_term_globally ha trovato {len(data_list)} possessori"
                )
                return data_list
    ...
    def get_possessori_per_partita(self, partita_id: int) -> List[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                self.logger.info(
                    f"Trovati {len(results)} possessori per la partita ID {partita_id}"
                )
                return results
    ...
    def get_possessori_by_comune(
        ...
        if filter_text:
            where_clauses.append(
                "(p.nome_completo ILIKE %s OR p.cognome_nome ILIKE %s)"
            )
        ...
        try:
            ...
            self.logger.error(
                f"Errore DB in get_possessori_by_comune: {e}", exc_info=True
            )
    ...
    def update_possessore(self, possessore_id: int, dati_modificati: Dict[str, Any]):
        ...
        if not dati_modificati:
            self.logger.info(
                f"Nessun dato fornito per aggiornare possessore ID {possessore_id}."
            )
            return
        ...
        if not set_clauses:
            self.logger.info(
                f"Nessun campo valido da aggiornare per possessore {possessore_id}."
            )
            return
        ...
        try:
                ...
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        raise DBNotFoundError(
                            f"Nessun possessore trovato con ID {possessore_id} da aggiornare."
                        )
            ...
            self.logger.error(
                f"Errore previsto aggiornando possessore {possessore_id}: {e}",
                exc_info=True,
            )
            raise e
        except Exception as e:
            self.logger.error(
                f"Errore imprevisto DB aggiornando possessore {possessore_id}: {e}",
                exc_info=True,
            )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    if possessore_data:
                        self.logger.info(
                            f"Dettagli recuperati per il possessore ID {possessore_id}."
                        )
                        return dict(possessore_data)
                    else:
                        self.logger.warning(
                            f"Nessun possessore trovato con ID {possessore_id}."
                        )
                        return None
        except Exception as e:
            self.logger.error(
                f"Errore DB in get_possessore_full_details per ID {possessore_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    def aggiungi_possessore_a_partita(
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if not new_relation_id:
                        raise DBMError(
                            "Inserimento del legame fallito, nessun ID restituito."
                        )

            self.logger.info(
                f"Possessore ID {possessore_id} associato a partita ID {partita_id}. ID Relazione: {new_relation_id}."
            )
            return True
            ...
            self.logger.error(
                f"Errore imprevisto in aggiungi_possessore_a_partita: {e}",
                exc_info=True,
            )
    ...
    def rimuovi_possessore_da_partita(self, partita_possessore_id: int) -> bool:
        ...
        if not (isinstance(partita_possessore_id, int) and partita_possessore_id > 0):
            raise DBDataError(
                f"ID relazione partita-possessore non valido: {partita_possessore_id}"
            )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount == 0:
                        ...
                        self.logger.warning(
                            f"Tentativo di rimuovere legame ID {partita_possessore_id} non trovato."
                        )
                        raise DBNotFoundError(
                            f"Nessun legame partita-possessore trovato con ID {partita_possessore_id}."
                        )

            # Il commit è automatico qui se l'operazione ha successo
            self.logger.info(
                f"Legame partita-possessore ID {partita_possessore_id} rimosso con successo."
            )
            return True

        except (DBNotFoundError, DBDataError) as e:
            self.logger.error(
                f"Errore previsto rimuovendo legame {partita_possessore_id}: {e}",
                exc_info=True,
            )
            raise e  # Rilancia l'eccezione specifica
        except Exception as e:
            self.logger.error(
                f"Errore imprevisto rimuovendo legame {partita_possessore_id}: {e}",
                exc_info=True,
            )
            raise DBMError(f"Impossibile rimuovere il legame: {e}") from e

    def get_possessore_data_for_export(
        self, possessore_id: int
    ) -> Optional[Dict[str, Any]]:
        """
            ...
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    if result and result["possessore_data"] is not None:
                        self.logger.info(
                            f"Dati per esportazione recuperati per possessore ID {possessore_id}."
                        )
                        return result["possessore_data"]
                    else:
                        self.logger.warning(
                            f"Nessun dato di export per possessore ID {possessore_id}."
                        )
                        return None
        except Exception as e:
            self.logger.error(
                f"Errore DB in get_possessore_data_for_export (ID: {possessore_id}): {e}",
                exc_info=True,
            )
            return None
                ...
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    if result and result["possessore_json"]:
                        try:
                            ...
                            return json.dumps(
                                result["possessore_json"], indent=4, ensure_ascii=False
                            )
                        except (TypeError, ValueError) as json_err:
                            self.logger.error(
                                f"Errore serializzazione JSON per possessore {possessore_id}: {json_err}"
                            )
                            return str(result["possessore_json"])
            self.logger.warning(
                f"Nessun JSON restituito per possessore ID {possessore_id}."
            )
        except psycopg2.Error as db_err:
            self.logger.error(
                f"Errore DB export_possessore_json (ID: {possessore_id}): {db_err}"
            )
        except Exception as e:
            self.logger.error(
                f"Errore Python export_possessore_json (ID: {possessore_id}): {e}"
            )
        return None

    def ricerca_avanzata_possessori(
        self, query_text: str, similarity_threshold: Optional[float] = 0.2
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    results = [dict(row) for row in cur.fetchall()]
                    self.logger.info(
                        f"Ricerca avanzata possessori per '{query_text}' ha prodotto {len(results)} risultati."
                    )
                    return results
        except Exception as e:
            self.logger.error(
                f"Errore DB durante la ricerca avanzata dei possessori: {e}",
                exc_info=True,
            )
