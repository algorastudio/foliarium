...
class DBAuditMixin:
    ...
    def set_session_app_user(
        ...
        """
        self.logger.debug(
            f"Impostazione var sessione: app.user_id='{user_id}', app.ip_address='{client_ip}'"
        )
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        "SELECT set_config('app.user_id', %s, false);", (user_id_str,)
                    )
                    cur.execute(
                        "SELECT set_config('app.ip_address', %s, false);", (ip_str,)
                    )

            # Il commit è automatico all'uscita del blocco 'with' senza errori
            self.logger.info(
                "Variabili di sessione applicative impostate con successo."
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Errore DB impostando var sessione applicative: {e}", exc_info=True
            )
    ...
    def clear_session_app_user(self):
        ...
        self.logger.info(
            "Reset variabili di sessione applicative (app.user_id, app.ip_address)."
        )
    ...
    def get_audit_log(
        ...
        try:
            ...
            if operazione and operazione.upper() in ["I", "U", "D"]:
                ...
                params.append(operazione.upper())
            ...
            if data_fine:
                ...
                conditions.append("timestamp <= %s")
                params.append(data_fine_end_day)
            if utente_db:
            ...
            self.logger.error(f"Errore DB get_audit_log: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_audit_log: {e}")
            return []
        return []
    ...
    def get_record_history(self, tabella: str, record_id: int) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Errore DB get_record_history: {db_err}")
            return []
        except Exception as e:
            self.logger.error(f"Errore Python get_record_history: {e}")
            return []
        return []
    ...
    def get_audit_logs(
        ...
        try:
            ...
            self.logger.error(
                f"Errore durante il recupero dei log di audit: {e}", exc_info=True
            )
            return [], 0

        return logs, total_records

    def set_audit_session_variables(
        self, app_user_id: Optional[int], session_id: Optional[str]
    ) -> bool:
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        ...
                        (f"{self.schema}.app_user_id", str(app_user_id)),
                    )
                    cur.execute(
                        "SELECT set_config(%s, %s, false);",
                        (f"{self.schema}.session_id", session_id),
                    )

            # Il commit è gestito automaticamente dal context manager _get_connection
            self.logger.info(
                f"Variabili di sessione per audit impostate: app_user_id={app_user_id}, session_id={session_id[:8]}..."
            )
            return True

        except Exception as e:
            self.logger.error(
                f"Errore DB impostando variabili audit: {e}", exc_info=True
            )
            return False
    ...
    def clear_audit_session_variables(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Errore DB resettando variabili audit: {e}", exc_info=True
            )
            return False
    ...
    def cleanup_audit_logs(self, days_to_keep: int) -> int:
        ...
        if not isinstance(days_to_keep, int) or days_to_keep < 0:
            raise DBDataError(
                "Il numero di giorni da conservare deve essere un intero non negativo."
            )
        ...
        try:
            ...
            self.logger.error(
                f"Errore durante la pulizia dei log di audit: {e}", exc_info=True
            )
    ...
    def close_user_session(self, session_id: str) -> bool:
        ...
        if not session_id:
            self.logger.warning(
                "Nessun ID di sessione fornito, impossibile chiudere la sessione nel DB."
            )
            return False
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if cur.rowcount > 0:
                        ...
                        self.logger.warning(
                            f"Sessione {session_id} non trovata o già chiusa."
                        )
            return True
        except Exception as e:
            self.logger.error(
                f"Errore DB durante la chiusura della sessione {session_id}: {e}"
            )
            return False

    def registra_nuova_consultazione(
        self,
        ...
        if not all([data_consultazione, richiedente, materiale_consultato]):
            raise DBDataError(
                "Data, Richiedente e Materiale Consultato sono campi obbligatori."
            )
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if result and result[0] is not None:
                        new_id = result[0]
                        self.logger.info(
                            f"Nuova consultazione registrata con successo. ID: {new_id}"
                        )
                        ...
                        raise DBMError(
                            "Fallimento registrazione consultazione: nessun ID restituito."
                        )
        except Exception as e:
            self.logger.error(
                f"Errore DB in registra_nuova_consultazione: {e}", exc_info=True
            )
    ...
    def registra_consultazione(
        self,
        data: date,
        richiedente: str,
        documento_identita: Optional[str],
        motivazione: Optional[str],
        materiale_consultato: Optional[str],
        funzionario_autorizzante: Optional[str],
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Consultazione registrata — richiedente: '{richiedente}', data: {data}"
            )
            return True
        except psycopg2.Error as db_err:
            self.logger.error(f"Errore DB registrazione consultazione: {db_err}")
            return False
        except Exception as e:
            self.logger.error(f"Errore Python registrazione consultazione: {e}")
            return False
    ...
    def update_consultazione(self, consultazione_id: int, **kwargs) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Errore DB aggiornamento consultazione ID {consultazione_id}: {db_err}"
            )
            return False
        except Exception as e:
            self.logger.error(
                f"Errore Python aggiornamento consultazione ID {consultazione_id}: {e}"
            )
            return False
    ...
    def delete_consultazione(self, consultazione_id: int) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Errore DB eliminazione consultazione ID {consultazione_id}: {db_err}"
            )
            return False
        except Exception as e:
            self.logger.error(
                f"Errore Python eliminazione consultazione ID {consultazione_id}: {e}"
            )
            return False

    def search_consultazioni(
        self,
        data_inizio: Optional[date] = None,
        data_fine: Optional[date] = None,
        richiedente: Optional[str] = None,
        funzionario: Optional[str] = None,
    ) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Errore DB in search_consultazioni: {db_err}")
        except Exception as e:
            self.logger.error(f"Errore Python in search_consultazioni: {e}")
        return []
    ...
    def get_recent_session_logs(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(
                f"Errore durante il recupero dei log di sessione recenti: {e}",
                exc_info=True,
            )
            return []
